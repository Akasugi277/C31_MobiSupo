...
import { addDays, format, format as formatDate, getDay, parseISO, subWeeks } from 'date-fns';
...
import { Agenda, DateData, Calendar as RNCalendar } from 'react-native-calendars';
...
export default function CalendarScreen() {
	...
	// ダミー予定は一旦空にする（実際は API 取得で上書き）
	const items = useMemo(() => ({} as Record<string, { id: string; time: string; title: string }[]>), []);
		...
		<SafeAreaView style={[styles.safe, { backgroundColor: bgColor }]} edges={['top', 'bottom']}>
			<View style={styles.segmentContainer}>
				{(['day', 'week', 'month'] as ViewMode[]).map((m) => (
					...
					>
						<Text style={[styles.segmentText, { color: mode === m ? '#fff' : textColor }]}>
...
function WeekView({
	...
	const weekRangeText = useMemo(() => {
		...
		return `${format(start, 'yyyy年M月d日', { locale: ja })}～${format(end, 'yyyy年M月d日', { locale: ja })}週`;
	}, [weekDates]);
			...
			<View style={[styles.weekStrip]}>
				{weekDates.map((d) => {
					...
					return (
						...
						>
							<Text style={{ color: selected ? '#fff' : dayColor }}>{dayNum}</Text>
							<Text style={{ color: selected ? '#fff' : dayColor, fontSize: 12 }}>{dayText}</Text>
							{has && <View style={[styles.dot, { backgroundColor: selected ? '#fff' : '#ff0' }]} />}
						</TouchableOpacity>
					);
				})}
			</View>
			...
			<View style={{ marginTop: 8 }}>
				{(items[selectedDate] || []).length === 0 ? (
					...
					(items[selectedDate] || []).map((it) => (
						<ShadowView key={it.id} style={[styles.itemBox, { backgroundColor: bgColor }]}>
							<Text style={{ color: textColor }}>{it.time} {it.title}</Text>
						</ShadowView>
					))
				)}
			</View>
		...
		<>
			...
			<RNCalendar
				...
				onMonthChange={dateObj => {
					setCurrentMonth(`${dateObj.year}-${String(dateObj.month).padStart(2, '0')}`);
				}}
